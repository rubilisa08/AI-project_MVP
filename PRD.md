...
- **URL 입력**: URL을 서버가 fetch한 뒤 응답의 `Content-Type`으로 분기한다(URL 문자열의 `.pdf` 확장자 여부는 보지 않음 — DART 등 공시 사이트는 확장자 없는 뷰어/다운로드 링크를 쓰는 경우가 많음).
  - `application/pdf`이면 업로드된 PDF와 완전히 동일한 경로(텍스트 청크 + 표 추출)로 처리. 파일명은 `Content-Disposition` 헤더 또는 URL 마지막 경로 세그먼트에서 유추.
  - 그 외(HTML)면 기존 뉴스 기사 파싱: `<script>/<style>/<nav>/<footer>/<header>/<noscript>/<iframe>/<form>` 제거 후 `<article>` 우선 추출(200자 미만이면 `<body>` 전체로 대체). 문단 단위로 청크 분할, 위치 라벨은 `"본문 N문단"`.
  - 파일 업로드 드롭존은 실제 파일 드롭뿐 아니라 **다른 탭에서 끌어온 링크 드롭**도 인식해 URL 목록에 자동 추가한다(`components/FileDropzone.tsx`) — 클릭 한 번으로 DART 조회 화면의 보고서 링크를 그대로 끌어다 놓을 수 있음.
